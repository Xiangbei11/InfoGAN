...
class Generator(nn.Module):
    ...
    def forward(self, zc):
        ...
        return curve_camber_thickness_fake#, r_adjacent, r_nonintersection
...
def train(net_D, net_G, data_iter, num_epochs, lr_D, lr_G, lr_info, noise_dim, latent_dim):
    ...
    for epoch in range(num_epochs):
        for i, (Xy,Xc) in enumerate(data_iter):
            ...
            G_Xy = net_G(torch.cat((Z,C),-1))
            _, pred_c = net_D(G_Xy)
            loss_info = lambda_con * continuous_loss(pred_c, C)
            loss_info.backward()
            trainer_info.step() 
...
lr_D, lr_G,lr_info, num_epochs = 0.00001, 0.00001, 0.00001, 6000 #50
test_str = '_camber_thickness_test1_' + "%s"%num_epochs
...
for latent_c in range(latent_dim):
    ...
    fake_data_test = Gen(torch.cat((Z,C),-1))
