...
geometry_loss = torch.nn.MSELoss() 
...
def train(net_D, net_G, data_iter, num_epochs, lr_D, lr_G, latent_dim):
    ...
    for epoch in range(num_epochs):
        for i, (Xy,Xc) in enumerate(data_iter):
            ...
            curve = torch.cat((upper_curve_geo,lower_curve_geo),-1)
            geo_loss = lambda_geo*geometry_loss(G_geo, curve)
            loss_G = adversarial_loss(validity, ones.reshape(validity.shape)) + geo_loss
            loss_G.backward()
            trainer_G.step()
        ...
        loss_geo_list.append(geo_loss.item())

# Comment out lines 177 to 187 if you just want to run a trained generator
lr_D, lr_G,lr_info, num_epochs = 0.00001, 0.00001, 0.00001, 5000 #50
test_str = '_modified_GAN_test0_' + "%s"%num_epochs
