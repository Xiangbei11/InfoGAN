...
def train(net_D, net_G, data_iter, num_epochs, lr_D, lr_G, lr_info, noise_dim, latent_dim):
    ...
    for epoch in range(num_epochs):
        for i, (Xy,Xc) in enumerate(data_iter):
            ...
            loss_G = adversarial_loss(validity, ones.reshape(validity.shape)) + lambda_fft * adversarial_loss(G_latent, C)
            loss_G.backward()
            trainer_G.step()
...
test_str = '_camber_thickness_modified_test1_1e-5_' + "%s"%num_epochs
