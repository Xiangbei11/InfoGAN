...
num_training_samples = control_camber_thickness.shape[0]
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.G_net = nn.Sequential(nn.Linear(G_input_dim, 60), nn.ReLU(), 
            nn.Linear(60, 80), nn.ReLU(),
    ...
    def forward(self, zc):
        ...
        return curve_camber_thickness_fake
...
geometry_loss = torch.nn.MSELoss(reduction='sum') 

lambda_geo = 1

C_geo = torch.normal(0, 1, size=(num_training_samples, latent_dim), requires_grad=True)

upper_curve = curve_camber_thickness[:,0:301]+0.5*curve_camber_thickness[:,301:602]
lower_curve = curve_camber_thickness[:,0:301]-0.5*curve_camber_thickness[:,301:602]
curve = np.concatenate((upper_curve,lower_curve),axis = 1)
print('curve',curve.shape)
data_curve = torch.from_numpy(curve)
...
def train(net_D, net_G, data_iter, num_epochs, lr_D, lr_G, latent_dim):
    ...
    trainer_geo = torch.optim.Adam(net_G.parameters(), lr=1e-6)
    for epoch in range(num_epochs):
        for i, (Xy,Xc) in enumerate(data_iter):
            ...
            real_pred = net_D(Xy)
            fake_pred = net_D(G_Xy.detach())
            loss_D = (adversarial_loss(real_pred, ones.reshape(real_pred.shape)) +
                      adversarial_loss(fake_pred, zeros.reshape(fake_pred.shape))) / 2
            loss_D.backward()
            trainer_D.step()

            trainer_geo.zero_grad()
            G_geo = net_G(C_geo)
            upper_curve_geo = G_geo[:,0:301]+0.5*G_geo[:,301:602]
            lower_curve_geo = G_geo[:,0:301]-0.5*G_geo[:,301:602]
            curve_geo = torch.cat((upper_curve_geo,lower_curve_geo),-1) #(1525,601)
            geo_loss = geometry_loss(data_curve, curve_geo)/1525
            loss_geo = lambda_geo * geo_loss
            loss_geo.backward()
            trainer_geo.step()             

            if i % 80 == 0:           
                print( "[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f] [geo loss: %f]"
                % (epoch, num_epochs, i, len(data_iter), loss_D.item(), loss_G.item(), geo_loss.item()))
        loss_D_list.append(loss_D.item())
        loss_G_list.append(loss_G.item())
        loss_geo_list.append(loss_geo.item())
...
test_str = '_modified_GAN_test2_' + "%s"%num_epochs
generator_model_path =  'GAN_GENERATOR' +test_str

# train(net_D, net_G, data_iter, num_epochs, lr_D, lr_G, latent_dim)
# np.savetxt('txt_files/c_geo'+test_str+'.txt', C_geo.detach().numpy())

# fig, axs = plt.subplots(2, 1, figsize=(10,8))#
# axs[0].plot(range(num_epochs), loss_G_list)
# axs[0].plot(range(num_epochs), loss_D_list)
# axs[0].legend(['Generator loss', 'Discriminator loss'])
# axs[1].plot(range(num_epochs), loss_geo_list)
# axs[1].legend(['Geometry loss'])
...
C_geo_saved = np.loadtxt('txt_files/c_geo'+test_str+'.txt')
fake_data_geo = Gen(torch.from_numpy(C_geo_saved).float())
fake_data_geo = fake_data_geo.detach().numpy()
...
for i in range(test_size0):
    ...
    for j, c in zip(range(test_size1), color):
        axs[i].plot(curve_x_coords[0:301,0].reshape((301,1)), (curve_camber_thickness[test_size1*i+j,0:301]+0.5*curve_camber_thickness[test_size1*i+j,301:602]).reshape((301,1)), c = c, lw = 5, alpha = 0.5, label = 'UIUC')
        axs[i].plot(curve_x_coords[0:301,0].reshape((301,1)), (curve_camber_thickness[test_size1*i+j,0:301]-0.5*curve_camber_thickness[test_size1*i+j,301:602]).reshape((301,1)), c = c, lw = 5, alpha = 0.5)
        axs[i].plot(curve_x_coords[0:301,0].reshape((301,1)), (fake_data_geo[test_size1*i+j,0:301]+0.5*fake_data_geo[test_size1*i+j,301:602]).reshape((301,1)), c = c, label = 'Fake')
        axs[i].plot(curve_x_coords[0:301,0].reshape((301,1)), (fake_data_geo[test_size1*i+j,0:301]-0.5*fake_data_geo[test_size1*i+j,301:602]).reshape((301,1)), c = c)  
    if i == 0:
        axs[i].legend(loc = 'upper right')
fig.tight_layout()
plt.savefig('figures/Comparison'+test_str)
# plt.show()
# exit()
...
# fake_data_test = fake_data_test.detach().numpy()
print('End')
fake_data_test = fake_data_geo#fake_data_test.detach().numpy()
test_size = num_training_samples
...
fig.tight_layout()
plt.savefig('figures/Comparison_Histogram_physical'+test_str)
...
# fig.legend(handles, labels, ncol= 2,loc='upper center')
fig.tight_layout()
plt.savefig('figures/Comparison_Histogram_fft_latent'+test_str)
plt.show()
exit()
